...
NeuronNet::NeuronNet(size_t _L, const vector <size_t> &nums) : L(_L) {				//nums[i] -- height of layers[i]
...
void NeuronNet::backpropagation(const vector <double> &input, const bool y, const double learning_rate) {
	...
	wdelt[0] = target_prime(y_hat, y) * sigmoid_prime(y_hat);
	...
	for (size_t l(L - 2); l > 0; --l) {
		...
		for (size_t k(0); k < wdelt.size(); ++k) {
			...
			wdelt[k] *= sigmoid_prime(activations[l][k]);
